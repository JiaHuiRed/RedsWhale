...
fn pricing_for_model(model: &str) -> Option<ModelPricing> {
    ...
    if lower.contains("v4-pro") || lower.contains("v4pro") {
        // DeepSeek V4 Pro — permanently reduced pricing.
        Some(ModelPricing {
            usd: CurrencyPricing {
...
mod tests {
    ...
    #[test]
    fn v4_pro_uses_permanent_lower_rates() {
        let pricing = pricing_for_model("deepseek-v4-pro").unwrap();
    ...
    fn v4_flash_keeps_current_published_rates() {
        let pricing = pricing_for_model("deepseek-v4-flash").unwrap();
